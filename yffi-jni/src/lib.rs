...
#[no_mangle]
pub extern "system" fn Java_com_yjava_bridge_NativeBridge_ydocNew(
    _env: JNIEnv,
    _class: JClass,
) -> jlong {
...
#[no_mangle]
pub unsafe extern "system" fn Java_com_yjava_bridge_NativeBridge_ydocWriteTransaction(
    mut env: JNIEnv,
    _class: JClass,
    doc_ptr: jlong,
    origin: jstring,
...
/*
#[no_mangle]
pub unsafe extern "system" fn Java_com_yjava_bridge_NativeBridge_ymapInsert(
    mut env: JNIEnv,
    _class: JClass,
    map_ptr: jlong,
    txn_ptr: jlong,
    key_str: jstring,
    input_arr: jbyteArray,
){
    let map = map_ptr as *const Branch;
    let txn = txn_ptr as *mut Transaction;
    let jstring = JString::from_raw(key_str);
    let rust_string = match env.get_string(&jstring) {
        Ok(s) => s,
        Err(_) => return,
    };

    let name_utf8: String = match rust_string.to_str() {
        Ok(s) => s.to_string(),
        Err(_) => return,
    };

    let c_string = match CString::new(name_utf8) {
        Ok(c) => c,
        Err(_) => return,
    };
    ...
    // 1. jbyteArray → Vec<u8>
    let jbytes = JByteArray::from_raw(input_arr);
    let vec = match env.convert_byte_array(jbytes) {
        Ok(v) => v,
        Err(_) => {
            return;
        }
    ...
    // 2. 길이 검사
    if vec.len() != std::mem::size_of::<YInput>() {
        return;
    }

    // 3. Vec<u8> → YInput 구조체로 reinterpret
    let mut raw = [0u8; std::mem::size_of::<YInput>()];
    raw.copy_from_slice(&vec);

    let yinput = raw.as_ptr() as *const YInput;

    ymap_insert(map, txn, c_string.as_ptr(), yinput);
}


#[no_mangle]
pub unsafe extern "system" fn Java_com_yjava_bridge_NativeBridge_ymapGet(
    mut env: JNIEnv,
    _class: JClass,
    map_ptr: jlong,
    txn_ptr: jlong,
    ...
    let name = match CString::new(name_utf8) {
        Ok(c) => c,
        Err(_) => return null_mut(),
    };

    let output = ymap_get(map, txn, name.as_ptr());
    if output == null_mut() {
        return null_mut();
    }
    let output_ref = &*output;
    let x = output_ref.value.str;
...
#[no_mangle]
pub unsafe extern "system" fn Java_com_yjava_bridge_NativeBridge_ymapRemove(
    mut env: JNIEnv,
    _class: JClass,
    map_ptr: jlong,
    txn_ptr: jlong,
    ...
    let key = match CString::new(name_utf8) {
        Ok(c) => c,
        Err(_) => return 0,
    };

    ymap_remove(map, txn, key.as_ptr()) as jbyte
}


#[no_mangle]
pub unsafe extern "system" fn Java_com_yjava_bridge_NativeBridge_yinputString(
    mut env: JNIEnv,
    _class: JClass,
    str: jstring,
)-> jbyteArray {
    let jstring = JString::from_raw(str);
    let rust_string = match env.get_string(&jstring) {
        Ok(s) => s,
        Err(_) => return null_mut(),
    };
    ...
    let yinput = yinput_string(key.as_ptr());

    // 3. 구조체를 &[u8] 슬라이스로 변환
    let struct_ptr = &yinput as *const YInput as *const u8;
    let struct_size = std::mem::size_of::<YInput>();
    let bytes: &[u8] = std::slice::from_raw_parts(struct_ptr, struct_size);
